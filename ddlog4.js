(function () {
  window.startDataLayerLogger = function (settings) {
    ...
    function getRootDomain(hostname) {
      ...
      // dev / localhost / ip -> ingen domain-attribut
    ...
    function buildEventPayload(event_name, data, isKey) {
      ...
      // sampled_session = kun når sampling er slået til, session ikke er i sample,
      // og data sendes pga key event (ikke pga session bucket)
      if (
        sampling_enabled &&
    ...
    function flush() {
      ...
      const hasKeyEvent = buffer.some(function (e) {
    ...
    /* ---------------- CMP (separat, 1x, exit-only, ignorer consent) ---------------- */
